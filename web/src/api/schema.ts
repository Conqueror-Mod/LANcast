...
export interface components {
    schemas: {
        ...
        Settings: {
            ...
            /** @description Base64 SHA-256 of this server's TLS certificate public key: the TLS certificate fingerprint, in the form a desktop client pins. It is not the server's identity (see GET /api/identity), and it is reissued for ordinary reasons -- a regenerated certificate, a rotated supplied one -- so a change in it is not by itself evidence of anything wrong (ADR 0070 as amended). Its use is first contact, before a session exists to read the identity over. Not a secret: a hash of a public key handed to anyone who opens a TLS connection. Empty on a loopback-only server. Follows a supplied certificate when tls_cert_file is set. */
