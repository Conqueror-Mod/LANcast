...
**Status:** accepted — AGPL-3.0 with a commercial licence available
**Date:** 2026-08-31
...
**AGPL-3.0-or-later, with a commercial licence available.** Chosen 2026-08-31.
...
Implemented in the same change: `LICENSE` carries the canonical AGPL-3.0 text,
`COMMERCIAL.md` states the second option and why contributions need a CLA,
`NOTICE` carries the attribution Apache-2.0 requires to travel, and the settings
page links to the source.

**That last one is section 13, not decoration.** A modified version offered to
people over a network must give those people its source, and this server is
exactly that shape — a thing other people in the house use through a browser.
Shipping the affordance means a fork inherits compliance rather than having to
remember it, which is the only version of this that works in practice.
