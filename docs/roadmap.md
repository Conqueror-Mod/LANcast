...
**Nothing sits unreleased.** v0.8.19 repaired v0.8.18.

**Three of the five features in v0.8.18 shipped built but unreachable**, and
its release notes described one of them as though it worked. History reset had
a store, two endpoints and no interface — usable only with curl. "Are you still
watching" rendered into chrome that is `opacity: 0` while the player is idle,
so it was invisible at exactly the moment it fired. Cast search learned to span
every library and no route could express it.

None of that was caught by 42 Go packages and 418 client tests, because **each
layer was correct about the half it owned**. A store with no caller compiles,
vets and tests clean; so does an endpoint with no interface. The suite had no
way to ask "is this wired to anything", which is the question that was missing —
and the previous entry here saying the features had not been run in the app was
the warning, written and then not acted on.

It was found by somebody opening Settings and looking for the feature.
