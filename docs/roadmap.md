...
**Nothing sits unreleased.** v0.8.18 shipped five backlog features and two
fixes found while building them.

**None of the five was run in the app before it shipped**, and that is recorded
here rather than in a commit nobody re-reads. Driving a dev build needed the
service stopped and the elevation for it never happened, so the evidence is the
test suite — which by this project's own repeated finding is the evidence that
misses things. The specific gaps: nothing has watched enrichment fetch a
headshot end to end, and jsdom sees no layout, so the cast row and the "still
watching" prompt have been reasoned about rather than looked at.
